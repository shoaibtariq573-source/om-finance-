            ...
            {!showDeploymentPanel ? (
              ...
              >
                <div className="flex items-center gap-2.5">
                  <span className="p-1 px-2.5 rounded-lg bg-blue-500/10 text-blue-400 text-[10px] font-bold uppercase tracking-wider">Deploy System</span>
                  <span className="text-xs font-bold text-slate-250">
                    🚀 Want to connect custom Domain (.om) & Web Hosting on GitHub? Click here to reveal the setup panel.
                  </span>
                </div>
                <div className="flex items-center gap-1 text-xs text-blue-400 font-bold hover:underline">
                  <span>Open Hub</span>
                  <ChevronDown className="w-4 h-4" />
                </div>
              </div>
            ) : (
              <div className="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-md space-y-4">
                {/* Header block with background gradient */}
                <div className="bg-slate-900 text-white p-5 px-6 flex items-center justify-between select-none">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-xl">
                      <Settings className="w-4.5 h-4.5" />
                    </div>
                    <div>
                      <h3 className="font-space font-extrabold text-sm md:text-base text-white">
                        Oman Hosting & Domain Management Hub 🌐
                      </h3>
                      <p className="text-[10px] sm:text-xs text-slate-400 font-medium tracking-wide">
                        Bilingual deploy blueprint to map custom Sultanate domains & push records to your GitHub
                      </p>
                    </div>
                  </div>
                  <button 
                    onClick={() => setShowDeploymentPanel(false)}
                    className="p-1 px-3 text-xs font-bold text-slate-400 hover:text-white bg-slate-800/50 hover:bg-slate-800 border border-slate-700/60 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
                  >
                ...
                {/* Submenu tabs */}
                <div className="px-6 pt-2 border-b border-slate-100 flex flex-nowrap overflow-x-auto gap-1">
                  <button
                    onClick={() => setActiveTab('download')}
                    className={`pb-3 px-3.5 text-xs font-bold transition-all border-b-2 whitespace-nowrap -mb-[1px] flex items-center gap-1.5 cursor-pointer ${
                      activeTab === 'download' 
                  ...
                  >
                    ...
                    3. Connect Custom Oman Domain (.om)
                  </button>
                  <button
                  ...
                  >
                    ...
                    4. Cloudflare Pages Setup ☄️
                  </button>
                </div>

                {/* Tab content panel */}
                <div className="p-6 pt-2 text-xs">
                  {activeTab === 'download' && (
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                      <div className="md:col-span-8 space-y-4">
                        <div className="space-y-1">
                          <h4 className="font-bold text-slate-900 text-sm">Download Source Code ZIP / Sync to GitHub</h4>
                          <p className="text-slate-500 leading-relaxed text-xs">
                            This application is compiled using React 18 with Vite. Google AI Studio allows direct file system extraction so you can host this code on any server.
                          </p>
                        </div>
                        
                        <div className="space-y-3">
                          <div className="flex gap-3.5 items-start">
                            <span className="w-6 h-6 rounded-full bg-slate-100 border border-slate-200 text-slate-700 font-bold flex items-center justify-center shrink-0">1</span>
                            <div>
                              <p className="font-bold text-slate-900">Locate Settings Icon ⚙️</p>
                              <p className="text-slate-500 text-[11px]">Look on the top right header of the AI Studio development environment control panel.</p>
                            </div>
                          </div>
                          
                          <div className="flex gap-3.5 items-start">
                            <span className="w-6 h-6 rounded-full bg-slate-100 border border-slate-200 text-slate-700 font-bold flex items-center justify-center shrink-0">2</span>
                            <div>
                              <p className="font-bold text-slate-900">Export Application</p>
                              <p className="text-slate-500 text-[11px]">
                                Click **"Export to GitHub"** to create an automated repository on your account, or **"Download ZIP"** to fetch a local offline archive of all files.
                              </p>
                            </div>
                          </div>
                        </div>
